    ...
    <div className={`space-y-8 ${textBase}`}>
      ...
      {sectionData.items.map((item, idx) => (
        <div key={idx} className={`relative pl-8 border-l-2 ${borderCol}`}>
          <div
            className={`absolute w-4 h-4 ${getColor(sectionData.color)} rounded-full -left-[9px] top-4`}
          />
          ...
          <div className="prose">
            ...
            <div className="mt-2">
              {item.reasonLeaving &&
                getReasonLeaving(item.reasonLeaving, undefined, isDark)}
            </div>
            ...
            {item.description && (
              <p className={`mt-2 leading-relaxed font-mono ${
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                {item.links.map(link => (
                  <a
                    ...
                    className={`px-3 py-1 rounded-full text-sm transition-colors ${linkBg}`}
                  >
          ...
          {idx !== sectionData.items.length - 1 && (
            <hr className={`my-6 border-2 ${
              isDark ? 'border-blue-700' : 'border-blue-900'
            }`} />
          )}
        </div>
      ))}
