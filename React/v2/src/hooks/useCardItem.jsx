...
export const CardItem = ({ project }) => {
  ...
  return (
    <div className="max-w-sm bg-white border border-red-200 shadow-sm dark:bg-gray-800 dark:border-red-700">
      <a href={project.link} target="_blank" rel="noopener noreferrer">
        {project.image.map(i => (<img key={i.id} className="rounded-t-lg w-full h-48 object-cover" src={i.href} alt={i.alt} />))}
      </a>
      <div className="px-6 py-4">
        <span className="flex justify-center gap-x-2 mt-2 mb-2  rounded-full text-center text-sm py-1 px-2 bg-[#1e1e1e]    dark:bg-yellow-700 dark:text-[#ffffff]">
      ...
      {project.links?.map(linkObj => {
          ...
          // Website link with dynamic label
          if (linkObj.website) {
            let label;
