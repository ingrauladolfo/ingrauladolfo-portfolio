...
const Timeline = ({ data, isDark }) => {
    ...
    const getColor = c => ({ blue: 'bg-blue-500', green: 'bg-green-500' }[c] || 'bg-gray-500')
    const handleDateRange = (start, end) => getDateRange(() => currentYear, start, end, isDark)
            ...
            {[data.education, data.work].map(section => {
                ...
                return (
                    <div key={section.title} className={orderClass}>
                        {renderSection({ ...section, items }, getWorkHere, handleDateRange, getColor, sectionFunc, getReasonLeaving, isDark)}
                        {showButton && (
                            <button onClick={onClick} className={`ml-12 py-3 px-4 flex items-center gap-3 ${btnTextClass} transition-colors rounded-lg`} >
