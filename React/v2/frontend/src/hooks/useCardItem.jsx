...
export const CardItem = ({ project, isDark }) => {
  ...
  // log solo al montar
  useEffect(() => {
    console.log(project.image.cover.href);
  }, []);

  // 1) retrasar 500ms la asignación del src
  const [delayedCover, setDelayedCover] = useState("");
  useEffect(() => {
    const t = setTimeout(() => setDelayedCover(project.image.cover.href), 3000);
    return () => clearTimeout(t);
  }, [project.image.cover.href]);

  // 2) controlar cuándo ya cargó la imagen
        ...
        <div className="w-full h-48 bg-black relative">
          {delayedCover && (
            <img
              ...
              onLoad={() => setTimeout(() => setLoadedCover(true), 50)}
              onError={() => setLoadedCover(false)}
              loading="lazy"
                className={`absolute top-0 left-0 w-full h-full object-cover object-top 
                transition-opacity duration-500 ease-in-out
               ${loadedCover ? "opacity-100" : "opacity-0"}`}
            />
          )}
        </div>
      </a>
          ...
          <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex gap-2">
            {images.map(img => (
              <button
                ...
                className={`w-2 h-2 rounded-full ${img.id === currentImageId
                    ? isDark
                      ? "bg-gray-400"
                      : "bg-gray-700"
                    : "bg-gray-300"
          ...
          if (linkObj.github) {
            items.push(
              <a
                ...
                className={`inline-flex items-center gap-1 px-2 py-1 text-sm rounded-xl border transition ${isDark
                    ? "bg-gray-800 border-gray-600 text-white hover:bg-red-800"
          ...
          if (linkObj.website) {
            ...
            items.push(
              <a
                ...
                rel="noopener noreferrer"
                className={`inline-flex items-center gap-1 px-2 py-1 text-sm rounded-xl border transition ${isDark
                    ? "bg-gray-800 border-gray-600 text-white hover:hover:bg-red-800"
                    : "bg-gray-800 border-blue-800 text-white hover:bg-gradient-to-r from-[#b84242] to-[#d20e0e]"
                  }`}
              >
                <FaLink /> {label}
